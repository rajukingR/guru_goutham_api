...
export const getApprovedProductSummary = async (req, res) => {
  try {
    ...
    // 5. Assembled components - CRITICAL FIX
    const assembledComponents = await AssembledComponent.findAll({
      attributes: ['asset_id', 'product_id']
    });

    // Create a Set of ALL asset IDs that are used in ANY assembly
    ...
    const result = productIdsFromGRN.map(productId => {
      ...
      // Get ALL assets for this product from GRN
      const allAssetsFromGRN = assetMap[productId] || [];
      
      // Filter out assets that are used in assemblies (regardless of which product)
      ...
      // Final used set - exclude returned items
      ...
      // Available assets: not used and not returned
      const availableAssetIds = assetsNotInAssemblies.filter(id => 
        !finalUsedSet.has(id)
      );

      // Calculate how many components of this product are used in assemblies
      ...
      const adjustedTotalQty = totalQty - componentQty;
      const usedQty = finalUsedSet.size;
      const product = productMap[productId];
      const purchasePrice = parseFloat(product?.purchase_price || 0);
    ...
    for (const asset of assembledAssets) {
      ...
      const availableAssetIds = isUsed ? [] : [parentAssetId];
      const purchasePrice = parseFloat(product.purchase_price || 0);
    ...
    // Sort result by product_id in descending order
...
export const updateGoodsReceipt = async (req, res) => {
  try {
    ...
    // 2. Get all incoming asset IDs from the update payload
    const incomingAssetIds = items.flatMap(item => item.asset_ids || []);

    // 3. Get existing asset_ids from all other goods receipts (exclude current)
    const existingItems = await GoodsReceiptItem.findAll({
      where: {
        goods_receipt_id: {
    ...
    // 4. Check for duplicate asset IDs per item
    const duplicateDetails = items.map(item => {
      const duplicates = (item.asset_ids || []).filter(id => existingAssetIds.includes(id));
      return {
        product_id: item.product_id,
    ...
    // 6. Update or create GoodsReceiptItems
    for (const item of items) {
    ...
    // 7. Optionally update AssetId table
    // You can remove old asset entries for this receipt and insert updated ones
