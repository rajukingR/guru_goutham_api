...
export const getAllInvoices = async (req, res) => {
  try {
    ...
    // Main query - remove subQuery: false or set it to true
    const rows = await Invoice.findAll({
      ...
      limit,
      offset,
      distinct: true,  // Keep this
      // subQuery: false,  // REMOVE THIS LINE - it's causing the issue
    });

    // Rest of your credit note logic...
    const dispatchOrderIds = rows
    ...
    const finalResult = rows.map(inv => {
...
//       // Additional challans - FIXED: Include ALL challans regardless of peripheral_update
//       // Remove the condition that filters out peripheral_update false
//       const otherChallans = await DeliveryChallan.findAll({
//         where: {
//           customer_code: invoice.customer_id.toString(),
//           dispatch_order_id: {
...
//             [Op.lt]: invoiceDcDate
//           },
//           // REMOVED: peripheral_update: false,
//           payment_type: invoice.payment_mode,
//         },
//         include: [{
//           model: DeliveryChallanItem,
...
//         // Process items and add device IDs
//         challanJSON.items.forEach((it) => {
//           const deviceIds = parseJSONSafe(it.device_ids);
//           invoiceDeviceIds.push(...deviceIds);
          
//           // Update item with filtered device IDs
//           const updatedDeviceIds = filterRemainingDevices(
//             deviceIds,
//             allReturnedForChallan,
//             allSwappedDeviceIds
...
//           it.device_ids = updatedDeviceIds;
//           it.quantity = updatedDeviceIds.length;
//         });
...
//         // Only add challan if it has items with devices
//         if (challanJSON.items.some(item => item.device_ids.length > 0)) {
//           additionalDeliveryChallans.push(challanJSON);
//         }
//       }
//     }
...
//     // Filter asset transactions based on payment mode
...
//       if (invoice.payment_mode === "Prepaid") {
//         // Prepaid: Show transactions from months BELOW (before) the invoice month
//         return (txnYear < invoiceYear) || 
//                (txnYear === invoiceYear && txnMonth < invoiceMonth);
//       } else if (invoice.payment_mode === "Postpaid") {
//         // Postpaid: Show transactions from the SAME month as invoice
//         return txnMonth === invoiceMonth && txnYear === invoiceYear;
//       }
      
//       // Default: return false if payment mode not recognized
//       return false;
//     });
...
//     console.log("Additional delivery challans count:", additionalDeliveryChallans.length);
//     console.log("Total device IDs:", invoiceDeviceIds.length);
...
export const getInvoiceById = async (req, res) => {
  try {
    ...
    // Check if return date is BEFORE invoice start date (for Prepaid mode)
    const isReturnBeforeInvoiceStart = (returnedDate, invoiceStartDate) => {
      if (!returnedDate || !invoiceStartDate) return false;
      const returnDate = new Date(returnedDate);
      const invoiceStart = new Date(invoiceStartDate);
      
      // Compare year and month only (ignore day)
      const returnYearMonth = returnDate.getFullYear() * 12 + returnDate.getMonth();
      const invoiceYearMonth = invoiceStart.getFullYear() * 12 + invoiceStart.getMonth();
      
      // Return true ONLY if return is in a month BEFORE invoice start month
      return returnYearMonth < invoiceYearMonth;
    };
    ...
    const countDispatchOrderUpToCurrent = async (dispatchOrderId, currentInvoiceId) => {
      ...
      return await Invoice.count({
        where: {
          dispatch_order_id: dispatchOrderId,
          id: { [Op.lte]: currentInvoiceId },
        },
      });
    };

    // Function to separate credit notes based on payment mode
    const separateCreditNotesByMonth = (creditNotes, invoiceStartDate, paymentMode) => {
      const invoiceStart = new Date(invoiceStartDate);

      const displayCreditNotes = [];  // For display only - NOT subtracted
      const subtractCreditNotes = []; // For subtraction
      ...
      creditNotes.forEach((note) => {
        ...
        if (paymentMode === "Prepaid") {
          // For Prepaid: ONLY subtract if return happened in a month BEFORE invoice start month
          if (isReturnBeforeInvoiceStart(returnedDate, invoiceStart)) {
            subtractCreditNotes.push(note);
          } else {
            displayCreditNotes.push(note);
          }
        } else {
          // Postpaid mode: Original logic based on month comparison
          const returnedMonth = returnedDate.getMonth();
          const returnedYear = returnedDate.getFullYear();
          const invoiceStartMonth = invoiceStart.getMonth();
          const invoiceStartYear = invoiceStart.getFullYear();
          
          if (returnedMonth === invoiceStartMonth && returnedYear === invoiceStartYear) {
            displayCreditNotes.push(note);
          } else {
            subtractCreditNotes.push(note);
          }
      ...
      return { displayCreditNotes, subtractCreditNotes };
    };
    ...
    let displayCreditNotes = [];
    let devicesToSubtract = [];
    let additionalDeliveryChallans = [];
    let filteredAssetSwaps = [];
    let allSwappedDeviceIds = [];
    ...
    if (invoice.transaction_type !== "Buy") {
      const allAssetSwaps = await AssetSwap.findAll({
        where: { swapped_on: { [Op.lte]: invoiceEnd } },
      });
      ...
      const allCreditNotes = await CreditNote.findAll({
        where: {
          dispatch_order_id: invoice.dispatch_order_id,
          transaction_type: { [Op.notIn]: ["Asset Swap", "Asset Removed"] }
        },
        include: [{ model: CreditNoteItem, as: "items" }],
      });

      const allCreditNotesJSON = allCreditNotes.map(note => note.toJSON());
      const { displayCreditNotes: displayNotes, subtractCreditNotes: subtractNotes } = separateCreditNotesByMonth(
        allCreditNotesJSON,
        invoice.invoice_start_date,
        invoice.payment_mode
      );

      displayCreditNotes = displayNotes;

      subtractNotes.forEach((note) => {
        if (note.transaction_type !== "Asset Swap" && note.transaction_type !== "Asset Removed") {
          note.items.forEach((ri) => {
            const returnedDevices = parseJSONSafe(ri.device_ids);
            devicesToSubtract.push(...returnedDevices);
          });
        }
      });
      ...
      for (const challan of otherChallans) {
        ...
        if (invoice.payment_mode === "Prepaid" && monthDiff(challanDate, invoiceStart) === 0) {
          continue;
        }
        ...
        const allChallanCreditNotes = challanCreditNotes.map(note => note.toJSON());
        const { subtractCreditNotes: challanSubtractNotes } = separateCreditNotesByMonth(
          allChallanCreditNotes,
          invoice.invoice_start_date,
          invoice.payment_mode
        );

        let devicesToSubtractForChallan = [];
        challanSubtractNotes.forEach((note) => {
          if (note.transaction_type !== "Asset Swap" && note.transaction_type !== "Asset Removed") {
            note.items.forEach((ri) => {
              devicesToSubtractForChallan.push(...parseJSONSafe(ri.device_ids));
            });
          }
        });
        ...
        challanJSON.credit_notes = challanSubtractNotes.length > 0 ? [] : challanCreditNotes;
        challanJSON.times_created_in_invoice = await countDispatchOrderUpToCurrent(
          challan.dispatch_order_id,
          invoice.id
        );
    ...
    let assetTransactions = await AssetTransaction.findAll({
      where: {
        customer_id: invoice.customer_id,
        parent_asset_id: { [Op.in]: invoiceDeviceIds }
      }
    });
    ...
    assetTransactions = assetTransactions.filter((txn) => {
      ...
      if (invoice.payment_mode === "Prepaid") {
        return (txnYear < invoiceYear) || (txnYear === invoiceYear && txnMonth < invoiceMonth);
      } else if (invoice.payment_mode === "Postpaid") {
        return txnMonth === invoiceMonth && txnYear === invoiceYear;
      }
    ...
    for (const item of invoice.items) {
      ...
      const remaining_device_ids = invoice.transaction_type === "Buy"
        ? device_ids
        : filterRemainingDevices(
            device_ids,
            [...returned_device_ids, ...devicesToSubtract],
            allSwappedDeviceIds
          );
    ...
    invoiceJSON.order_table_id = order_table_id;
    invoiceJSON.order_date = order_date;
    invoiceJSON.credit_notes = displayCreditNotes;
    invoiceJSON.asset_swaps = filteredAssetSwaps;
    invoiceJSON.additional_delivery_challans = additionalDeliveryChallans;
    invoiceJSON.rental_start_date = invoice.rental_start_date;
    invoiceJSON.rental_end_date = invoice.rental_end_date;
